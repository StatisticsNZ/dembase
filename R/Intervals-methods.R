...
setMethod("inferDimvalues",
          ...
          function(DimScale, labels, isAge, labelStart = TRUE) {
              ...
              if (identical(n, 0L))
                  return(numeric())
              try.to.interpet.as.time.units <- !isAge && (n >= 2L)
              if (try.to.interpet.as.time.units) {
                  if (isValidIntervalLabelsDate(labels)) {
                      dimvalues <- intervalDimvaluesFromDates(labels)
                      return(dimvalues)
                  }
                  if (isValidIntervalLabelsMonth(labels)) {
                      dimvalues <- intervalDimvaluesFromMonths(labels)
                      return(dimvalues)
                  }
                  if (isValidIntervalLabelsQuarter(labels)) {
                      dimvalues <- intervalDimvaluesFromQuarters(labels)
                      return(dimvalues)
                  }
              }
              ...
              if (!decoded.labels[n]) {
                  ...
                  ## single integer x.  If so, assume that the final interval
                  ## is [x, x+1) or [x-1, x). The decoding cannot be done
