...
#'
#' Mean Squared Error, or MSE, measures the extent to which
#' point estimates (including forecasts) from a model come
#' close to the truth.  It is commonly used in simulation
#' studies, where the true values are generated by the
#' analyst, and hence known.
#'
#' If \code{p} is a point estimate and \code{t} is the truth
#' then the MSE is \code{(p - t)^2}.
#'
#' \code{MSE} is stricter about the compatibility
#' of its arguments than most functions in \code{dembase}.
#' Although it reorders dimensions and categories
#' in \code{point} and \code{truth}, it does not collapse
#' or expand dimensions, or drop any levels.
#'
#' @param point A \code{\linkS4class{DemographicArray}} containing
#' the point estimates.
#' @param truth A \code{\linkS4class{DemographicArray}} containing
#' the true values.
#' 
#' @return An object with the same class as \code{truth}.
#'
#' @examples
...
#' if \code{l <= t <= u}. The use of \code{<=}, rather than
#' \code{<}, makes no difference
#' when \code{l} and \code{u} are real numbers, but does make
#' a difference when \code{l} and \code{u} are integers.
#'
#' \code{truth} can be a single number, in which case all dimensions
#' of \code{interval}, other than the quantile dimension, should
#' have length 1.
#'
#' \code{intervalContainsTruth} is stricter about the compatibility
#' of its arguments than most functions in \code{dembase}.
#' Although it reorders dimensions and categories
...
#' number.
#'
#' @return An \code{\linkS4class{Values}} object, consisting
...
#' Interval score, a measure of forecast quality
#'
...
#'
#' @inheritParams intervalContainsTruth
#'
#' @return A \code{\linkS4class{Counts}} object with the
