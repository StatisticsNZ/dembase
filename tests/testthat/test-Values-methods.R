...
test_that("resetDiagInner works", {
    ...
    ans.expected[c(1,4)] <- NA_integer_
    expect_identical(ans.obtained, ans.expected)
    ans.obtained <- resetDiag(object, reset = 0)
    ans.expected <- object
...
test_that("round3 works", {
    for (seed in 1:10) {
        ...
        x <- ValuesOne(rpois(n = 100, lambda = lambda),
                       labels = 1:100,
                       name = "reg")
        x.round <- round3(x)
        expect_true(all(x.round %% 3 == 0))
        expect_true(all(x[x %% 3 == 0L] == x.round[x %% 3 == 0L]))
        expect_true(is(x.round, "Values"))
        x.with.na <- ValuesOne(rpois(n = 100, lambda = lambda),
                               labels = 1:100,
                               name = "reg")
