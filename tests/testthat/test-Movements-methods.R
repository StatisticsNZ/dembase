...
test_that("accession works with Movements - with age, account contains births", {
    ...
    acc.births <- addDimension(acc.births,
                               ...
                               labels = "0",
                               dimscale = "Points")
    acc.internal <- slab(x@components[[2]],
                         dimension = "triangle",
                         elements = "TU")
    ...
    acc.internal <- slab(acc.internal,
                         dimension = "age",
                         elements = 1:2)
    acc.immigration <- slab(x@components[[3]],
                            dimension = "triangle",
                            elements = "TU")
    acc.immigration <- slab(acc.immigration,
                            dimension = "age",
                            elements = 1:2)
    acc.deaths <- slab(x@components[[4]],
                       dimension = "triangle",
                       elements = "TU")
    ...
    acc.emigration <- slab(x@components[[5]],
                           dimension = "triangle",
                           elements = "TU")
    ...
    acc.reclassification <- slab(x@components[[6]],
                                 dimension = "triangle",
                                 elements = "TU")
...
test_that("accession works with Movements - with age, account does not contain births", {
    ...
    x <- Movements(population = population,
                   ...
                   exits = list(deaths = deaths, emigration = emigration),
                   net = list(reclassification = reclassification))
    acc.internal <- slab(x@components[[1]],
                         dimension = "triangle",
                         elements = "TU")
    acc.internal <- collapseOrigDest(acc.internal)
    acc.internal <- slab(acc.internal,
                         dimension = "age",
                         elements = 1:2)
    acc.immigration <- slab(x@components[[2]],
                            dimension = "triangle",
                            elements = "TU")
    acc.immigration <- slab(acc.immigration,
                            dimension = "age",
                            elements = 1:2)
    acc.deaths <- slab(x@components[[3]],
                       dimension = "triangle",
                       elements = "TU")
    acc.deaths <- slab(acc.deaths,
                       dimension = "age",
                       elements = 1:2)
    acc.deaths <- -1L * acc.deaths
    acc.emigration <- slab(x@components[[4]],
                           dimension = "triangle",
                           elements = "TU")
    acc.emigration <- slab(acc.emigration,
                           dimension = "age",
                           elements = 1:2)
    acc.emigration <- -1L * acc.emigration
    acc.reclassification <- slab(x@components[[5]],
                                 dimension = "triangle",
                                 elements = "TU")
    acc.reclassification <- slab(acc.reclassification,
                                 dimension = "age",
...
test_that("isInternallyConsistent works with Movements - no age", {
    population <- CountsOne(values = seq(100, 200, 10),
                            labels = seq(2000, 2100, 10),
                            name = "time")
    births <- CountsOne(values = 15,
    ...
    x <- Movements(population = population,
                   births = births,
                   exits = list(deaths = deaths))
    expect_true(all(isInternallyConsistent(x)))
})
...
test_that("accession works with Movements - with age", {
    ...
    x <- Movements(population = population,
                   ...
                   exits = list(deaths = deaths, emigration = emigration),
                   net = list(reclassification = reclassification))
    expect_false(all(isInternallyConsistent(x)))
})
